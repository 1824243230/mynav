                               ...
                               Eigen::Vector3d end_pt, Eigen::Vector3d end_v, double end_yaw, bool init, bool dynamic, double time_start, bool gen_search)
{
  ...
  while (!open_set_.empty())
  {
    ...
    // 但其实one shot应该更多地被触发
    bool reach_horizon = (cur_node->state.head(2) - start_pt.head(2)).norm() >= ((topo_path_.size() >= 2) ? horizon_ : 2.0);
...
void KinodynamicAstar2D::setParam(ros::NodeHandle& nh)
{
  ...
  use_kino_replan_ = nh.param("search_2D/B_Spline_LocalPlanner", true);
  if (!use_kino_replan_) horizon_ = 2.0;
  useJPS_ = nh.param("search_2D/use_JPS", false);   // we do not use it now.
  {
...
double KinodynamicAstar2D::estimateHeuristic(Eigen::VectorXd x1, Eigen::VectorXd x2, double& optimal_time)
{
  ...
  if(topo_path_.size() >= 2)
  {
    ...
    float dis_to_goal = topo_path_distance_[pointIdxNKNSearch[0]];
    float dis_to_goal_start = topo_path_distance_[0];
    float dis_to_start = std::max(0.0f, dis_to_goal_start - dis_to_goal);

    // float dis_to_start = (x1.head(2) - start_pt_.head(2)).norm();
    float tt = ((dis_to_start - 0.45 * horizon_) / (0.45 * horizon_));
    tt = (tt < 0.0f) ? 0.0f : (tt > 1.0f ? 1.0f : tt);
    float ss = tt * tt * (3 - 2 * tt); // 0-1的平滑函数
    float w_disp = 0.1 + (0.9 - 0.1) * ss; // 0.1-1的权重
    {
      h_topo = dis_to_goal + w_disp * nearDistance;
    }
    h = h_topo;
  }
  // h = std::max(h, h_topo);
...
double KinodynamicAstar2D::calScaleFactor(const double& t, const double & v0, const double& curv_tmp)
{
  ...
  double acc_norm_2 = 0.25 * acc_normal * acc_normal + acc_tang * acc_tang; 
