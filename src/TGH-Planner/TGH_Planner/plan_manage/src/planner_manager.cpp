...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  src_file = file_path + "Utils/bspline_analysis/bspline_data/";
  local_data_.traj_id_ = 0;
  sdf_map_.reset(new SDFMap(nh));
  sdf_map_->initMap();
  ...
  record_file_.open(file_path + "Utils/traj_analysis/data/planning_time_cost.txt", std::ios::out);
                                        ...
                                        Eigen::Vector3d start_yaw, vector<Eigen::Vector3d>& start_change)
{
    ...
    Eigen::Vector3d start_pt_forward = start_pt;
