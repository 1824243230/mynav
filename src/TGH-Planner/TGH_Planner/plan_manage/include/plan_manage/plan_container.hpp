...
public:
  ...
  // 不仅存储了全局的信息，还储存了局部规划出来的信息，通过全局时间连接在一起
  // getPosition的时候，可以获取到当前执行的局部轨迹的信息，而不是全局路径的信息。
  // 设计得很巧妙？
...
struct PlanParameters {
  ...
  double ctrl_pt_dist;                   // distance between adjacient B-spline
                                         // control points
  double clearance_;
  int dynamic_;     //这个是表示当前的环境是不是动态的，如果是动态的，那么就要使用动态的信息
  /* processing time */
...
public:
  ...
  vector<Eigen::Vector3d> jps_path_;
  vector<Eigen::Vector3d> visited_nodes;
  vector<Eigen::Vector4d> search_tree;

  // topological paths
  list<GraphNode::Ptr> topo_graph_;
  vector<vector<Eigen::Vector3d>> topo_paths_;           // DFS raw paths
  vector<vector<Eigen::Vector3d>> topo_filtered_paths_;  // filtered paths
  vector<vector<Eigen::Vector3d>> topo_select_paths_;    // final paths
  vector<vector<Eigen::Vector3d>> voronoi_paths_;        // 
  vector<Eigen::Vector3d> topo_guide_path_;              // dubins shot final paths and select one best
  vector<Eigen::Vector3d> topo_sample_area_;
  ...
  void addTopoPaths(const list<GraphNode::Ptr>& graph, const vector<vector<Eigen::Vector3d>>& paths,
                    const vector<vector<Eigen::Vector3d>>& filtered_paths,
                    const vector<vector<Eigen::Vector3d>>& selected_paths) {
    topo_graph_ = graph;
    topo_paths_ = paths;
